...
class Mapping2Dto3D(nn.Module):
    ...
    def forward(self, x, latent):
        # print('initial x',x.shape)#initial x torch.Size([1, 2, 100])
        # print("latent vecotr", latent.shape)#latent vecotr torch.Size([32, 1024, 1])
        # print("self.conv1(x)",self.conv1(x).shape)#self.conv1(x) torch.Size([1, 1024, 100])
        # print(latent)
        x = self.conv1(x) + latent
        #print("x",x.shape) #x torch.Size([32, 1024, 100])
