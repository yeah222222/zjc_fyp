...
import os,sys
sys.path.append('/mnt/d/junch_data/projection_conditioned_point_cloud_diffusion/Projection_zjc/model')
...
class PointCloudProjectionModel(ModelMixin):
    ...
    def get_local_conditioning(self, image_rgb: Tensor, mask: Tensor):
        ...
        if self.use_local_colors:
            ...
            # print('image_rgb',self.normalize(image_rgb).shape)
        if self.use_local_features:
            local_conditioning.append(self.feature_model(image_rgb))
            # print('local feature',self.feature_model(image_rgb).shape)
        if self.use_mask:
            local_conditioning.append(mask.unsqueeze(1).float())
            # print('use mask',mask.shape)
